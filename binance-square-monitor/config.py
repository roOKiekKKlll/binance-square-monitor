...
# --- 风控硬限制 ---
TRADING_MAX_CONCURRENT_POSITIONS = 3     # $100 账户最多同时 3 个仓位
TRADING_MAX_DAILY_LOSS_PCT = 25.0        # 当日浮动+已实现亏损超该百分比则熔断停机（$100账户=亏$25）
TRADING_MAX_DAILY_TRADES = 10            # 当日最多开仓次数
...
LIVE_MAX_TOTAL_EXPOSURE_USD = 400.0      # 总敞口硬限（USD）
LIVE_ORDER_RETRY_COUNT = 3               # 下单失败重试次数
LIVE_ORDER_RETRY_DELAY_S = 1.0           # 重试间隔（秒）
LIVE_STOP_ORDER_RETRY_COUNT = 8          # 止损挂单网络失败时多试几次，避免频繁买入后立刻市价平仓
LIVE_STOP_ORDER_RETRY_DELAY_S = 2.0      # 止损挂单重试间隔（秒）
LIVE_EMERGENCY_CLOSE_ON_STOP_TRANSIENT_FAILURE = False  # 网络类止损失败不立即平仓，交给 live_manager 补挂
