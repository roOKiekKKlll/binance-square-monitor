...
TRADING_REVERSE_LONG_TO_SHORT = True # 当出现开多信号时，强行开空（止盈止损也会反转）
...
# 仓位 = 风险金额 / |entry - stop|
TRADING_SIZING_MODE = "fixed_margin"     # risk_based / fixed_margin
TRADING_RISK_PER_TRADE_PCT = 5.0         # 每笔最大风险占账户净值的比例（%）
TRADING_ORDER_AMOUNT = 13.0              # fixed_margin 模式下的固定保证金（13U x 10x = 约130U名义）
TRADING_FIXED_MARGIN_HALF_MULTIPLIER = 1.0  # fixed_margin 下 half 档保证金倍率（1.0=half 也按 13U 下单）
TRADING_MIN_NOTIONAL = 5.0               # 名义价值下限，低于此值不开仓（避免滑点放大）
TRADING_MAX_NOTIONAL_PCT = 50.0          # 单笔名义价值不超过账户净值的百分比

# --- 风控硬限制 ---
TRADING_MAX_CONCURRENT_POSITIONS = 8     # 最多同时 8 个仓位
TRADING_MAX_DAILY_LOSS_PCT = 50.0        # 当日浮动+已实现亏损超该百分比则熔断停机（$100账户=亏$25）
...
TRADING_OPEN_MAX_LOSS_PCT = 5.0          # 仅 OPEN 初始止损生效：单笔最大允许亏损（%）
...
# --- 市场状态过滤（Regime Filter，可开关）---
# 关闭时：完全不影响现有开仓逻辑。
# 开启时：仅当 benchmark（默认 BTC）处于趋势状态才允许自动开仓；
#         neutral / risk_off 一律拦截，避免在不适合做多的市场环境里反复止损。
TRADING_REGIME_FILTER_ENABLED = False
TRADING_REGIME_BENCHMARK_TOKEN = "BTC"
TRADING_REGIME_EXCLUDE_BENCHMARK_FROM_TRADING = True
TRADING_REGIME_TREND_MIN_1H_PCT = 0.2
TRADING_REGIME_TREND_MIN_4H_PCT = 0.8
TRADING_REGIME_TREND_MIN_OI_1H_PCT = -1.0
TRADING_REGIME_RISK_OFF_1H_PCT = -1.5
TRADING_REGIME_RISK_OFF_4H_PCT = -4.0
...
LIVE_MAX_TOTAL_EXPOSURE_USD = 1050.0     # 总敞口硬限（USD）
LIVE_RESIDUAL_POSITION_MAX_NOTIONAL_USD = 5.0  # DB 无仓位时允许自动清理的交易所残仓上限（USD）
